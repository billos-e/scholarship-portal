...
import { buildCsvZipBuffer } from "@/lib/export/csv-zip";
...
export async function POST(request: Request) {
  ...
  const buffer = await buildCsvZipBuffer(body.sheets as ExportSheet[]);

  return new Response(new Uint8Array(buffer), {
    headers: {
      "Content-Type": "application/zip",
      "Content-Disposition": `attachment; filename="${body.filename}.zip"`,
