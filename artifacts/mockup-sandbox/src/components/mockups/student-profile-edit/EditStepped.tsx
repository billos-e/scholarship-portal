...
function F({ label, value, type = "text", locked, wide }: { label: string; value: string; type?: string; locked?: boolean; wide?: boolean }) {
  return (
    <div className={`space-y-1.5 ${wide ? "col-span-2 lg:col-span-3" : ""}`}>
      <div className="flex items-center gap-1.5">
        <label className="text-xs font-medium text-muted-foreground">{label}</label>
        {locked && <Lock className="size-2.5 text-muted-foreground/40" />}
      </div>
      {locked ? (
        <div className="flex h-9 w-full items-center rounded-lg border border-dashed border-border/50 bg-muted/30 px-3 text-sm text-muted-foreground/60">{value}</div>
      ) : (
        <input type={type} defaultValue={value} className="h-9 w-full rounded-lg border border-border bg-background px-3 text-sm text-foreground outline-none focus:border-primary focus:ring-2 focus:ring-primary/15" />
      )}
    </div>
  );
}
...
function Sel({ label, value, wide }: { label: string; value: string; wide?: boolean }) {
  return (
    <div className={`space-y-1.5 ${wide ? "col-span-2 lg:col-span-3" : ""}`}>
      <label className="text-xs font-medium text-muted-foreground">{label}</label>
      <select className="h-9 w-full rounded-lg border border-border bg-background px-3 text-sm text-foreground outline-none focus:border-primary focus:ring-2 focus:ring-primary/15">
        <option>{value}</option>
      </select>
    </div>
  );
...
type Section = { id: number; title: string; summary: string; done: boolean; children: React.ReactNode };
    ...
    <div className="overflow-hidden rounded-2xl border border-border/70 bg-card shadow-sm divide-y divide-border/50">
      {sections.map((s) => (
        <div key={s.id}>
          ...
          >
            <div className={`flex size-8 shrink-0 items-center justify-center rounded-full text-xs font-bold transition ${s.done ? "bg-success text-white" : open === s.id ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}>
      ...
      children: (
        <div className="flex items-center gap-6">
          ...
          <div className="flex-1 space-y-2.5">
            <input type="file" accept="image/*" className="block w-full text-xs text-muted-foreground file:mr-2 file:rounded-md file:border file:border-border file:bg-card file:px-3 file:py-1 file:text-xs file:font-medium" />
      ...
      <div className="mx-auto max-w-5xl space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            ...
            <p className="text-sm font-bold text-foreground">1 / 4 complete</p>
          </div>
        </div>
        ...
        <div className="flex justify-end gap-3 pb-4">
          <button className="rounded-xl border border-border bg-card px-6 py-2.5 text-sm font-semibold text-foreground hover:bg-muted">Cancel</button>
          <button className="rounded-xl bg-accent px-7 py-2.5 text-sm font-bold text-accent-foreground hover:bg-accent/90">Save changes</button>
        </div>
      </div>
    </div>
  );
