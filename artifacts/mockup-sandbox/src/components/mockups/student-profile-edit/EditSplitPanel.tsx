...
function F({ label, value, type = "text", locked }: { label: string; value: string; type?: string; locked?: boolean }) {
  return (
    <div className="space-y-1.5">
      ...
      {locked ? (
        <div className="flex h-9 items-center rounded-lg border border-dashed border-border/50 bg-muted/30 px-3 text-sm text-muted-foreground/70">{value}</div>
      ) : (
      ...
      <aside className="flex w-64 shrink-0 flex-col border-r border-border/70 bg-card">
        ...
        <div className="flex flex-col items-center px-6 py-8 text-center">
          <div className="relative">
            ...
            <button className="absolute -bottom-1 -right-1 flex size-8 items-center justify-center rounded-full border-2 border-card bg-accent text-accent-foreground shadow-md hover:scale-110 transition">
        ...
        <div className="mx-4 mb-4 space-y-2 rounded-xl border border-border/60 bg-muted/30 p-4">
        ...
        <nav className="mt-auto border-t border-border/40 p-3 space-y-1">
          {["Personal information", "Academic information", "Bank information"].map((s, i) => (
            <div key={s} className={`flex items-center justify-between rounded-lg px-3 py-2 text-xs font-medium ${i === 0 ? "bg-primary/8 text-primary" : "text-muted-foreground hover:bg-muted"}`}>
              <span>{s}</span>
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto px-8 py-7 space-y-7">
          ...
          <Block title="Academic information">
            <div className="col-span-2"><Sel label="University" value={student.university} /></div>
